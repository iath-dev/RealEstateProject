...
const PropertyDialogContent: React.FC = () => {
  ...
  if (isLoading) return <Skeleton data-testid="skeleton" className="w-full rounded-lg h-18" />;

  if (!property || isError) return null;

  return (
    <div className="my-4 p-2 flex flex-col gap-4 starting:opacity-0 opacity-100 transition-all">
      <div data-testid="dialog-content-address" className="flex gap-4 items-center">
        <MapPin size={20} />
        <p className="text-gray-700 font-medium leading-relaxed">{property.address}</p>
      </div>
      <p
        data-testid="dialog-content-price"
        className="text-3xl font-bold text-end text-emerald-500"
      >
